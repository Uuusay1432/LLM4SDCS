...
def output_EnvReqInformation(context_content, context_response, input_env, input_req, input_reqenv, model):
    ...
    #content =  "The following is a summary of the relationship between an environment model and the actions it has in JSON format.\n\n" + input_env + "\n\nThe following is a summary of the relationship between an requirement model and the actions it has in JSON format.\n\n" + input_req + "\n\nFor each requirement model, the following is a summary in JSON format of the \"environment model that has the actions that the requirement model has\".\n\n" + input_reqenv

    content =  "I give you the LTS of the environment models following.\n\n" + envmodel  + "\n\nI give you requirement models following." + reqmodel + "\n\nAfter several syntheses, here is the current states of the environmental model we have now." + input_env  + "\n\nAfter several rounds of synthesis, here is the requirements model we have now." + input_req + "\n\nFor each requirement model, the following is a summary in JSON format of the \"environment model that has the actions that the requirement model has\".The set of environment models corresponding to the requirement model is called the monitored model.\n\n" + input_reqenv
...
def output_result(context_content, context_response, inf_content, inf_response, req1_content, req1_response, req2_content, req2_response, model):
    content = "Considering the totality of what you have analyzed so far, choose the best set of requirement models that should be synthesized at the beginning of the SDCS from current state in drone system, not in room system. And then, Create a set of environmental models to be synthesized with that requirement model."
    completion = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "user", "content": context_content},
